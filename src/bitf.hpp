...
public:
  ...
  {
    data<T>::_bits
        = func<T>::insert (value, index, func<T>::nofbits (value), bits);
  };
  /**
...
public:
  ...
  {
    return func<T>::get (data<T>::_bits, index, offset);
  };
  // get vector of n atomic values from bitdata
  virtual std::vector<T>
  get (int index, size_t offset, T n) const
  {
    return func<T>::get (data<T>::_bits, index, offset, n);
  };
  // get single bit value by index
  virtual T
  operator[] (int index) const
  {
    return func<T>::get (data<T>::_bits, index, 1);
  };
};
...
public:
  ...
  {
    data<T>::_bits = func<T>::insert (value, index, offset, data<T>::_bits);
  };
  ...
  {
    data<T>::_bits = func<T>::insert (values, index, offset, data<T>::_bits);
  };
};
} // namespace bitf
